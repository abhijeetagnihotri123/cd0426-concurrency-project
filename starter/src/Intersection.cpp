...
Intersection::~Intersection()
{
    std::cout << "Intersection #" << _id << " destructor called" << std::endl;
    // Signal the processVehicleQueue thread to stop
    _stopRequested = true;
    // The TrafficLight destructor will be called automatically since it's a member
    // The base class destructor will handle joining threads
}
